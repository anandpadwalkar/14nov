...
public class HomeController {
	...
	public ModelAndView adminPage() {
		 ...
		 * user=(SessionUser)SecurityContextHolder.getContext().
		 * getAuthentication().getPrincipal(); List<Roles>
		 * roles=user.getRoles(); logger.info("Role.....................size: "
		 * +roles.size()); for (Iterator iterator = roles.iterator();
		 * iterator.hasNext();) { Roles roles2 = (Roles) iterator.next(); int
		 * id= roles2.getId(); logger.info("Role.....................Id: " +id);
		 * }
		 */
		ModelAndView model = new ModelAndView();
		model.addObject("title", "Spring Security Hello World");
		...
		/*
		 * List<RoleMenuAllocation> menus
		 * =menuService.getMenuOfRole(testSession.getRoleId());
		 * model.addObject("menus", menus);
		 */
		/*
		 * List<Roles> list=menuService.getMenuOfRole(testSession.getRoleId());
		 * 
		 * model.addObject("menuList",list.get(0).getMenus()); Roles r = new
		 * Roles();r.setId(2); model.addObject("t",r);
		 */
